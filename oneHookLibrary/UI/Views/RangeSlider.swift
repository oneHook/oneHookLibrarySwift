// see: https://github.com/BrianCorbin/SwiftRangeSlider
...
@IBDesignable open class RangeSlider: UIControl {
    ...
    public let lowerKnob = RangeSliderKnob()
    public let upperKnob = RangeSliderKnob()
    let lowerLabel = CATextLayer()
    let upperLabel = CATextLayer()

    var aTrackThickness: CGFloat {
        trueTrackThickness ? trackThickness : trackThickness * bounds.height
    }

    var aKnobSize: CGFloat {
        trueKnobSize ? knobSize : knobSize * bounds.height
    }
    ...
    var knobsAreClose: Bool {
        lowerValue + trackRange * 0.05 >= upperValue
    }

    var knobsAreCloserToMinimum: Bool {
        maximumValue - upperValue > lowerValue - minimumValue
    }
    ...
    func addContentViews() {
        ...
        lowerKnob.frame = CGRect(x: 0, y: 0, width: aKnobSize, height: aKnobSize)
        lowerKnob.rangeSlider = self
        lowerKnob.contentsScale = UIScreen.main.scale
        layer.addSublayer(lowerKnob)

        upperKnob.frame = CGRect(x: 0, y: 0, width: aKnobSize, height: aKnobSize)
    ...
    open func makeSingleMode() {
        lowerKnob.isHidden = true
    ...
    open func updateTrackLayerFrameAndKnobPositions() {
        ...
        let newTrackDy = (frame.height - aTrackThickness) / 2
        track.frame = CGRect(x: 0, y: newTrackDy, width: frame.width, height: aTrackThickness)
        track.setNeedsDisplay()
    ...
    /**
     ...
     Breaking out this functionality from 'updateLabelText()' allows a subclass of RangeSlider to override this method and provide custom text.
     For example, if the slider is representing centimeters of snow, the override function could return "\(value)cm" instead of just "\(value)".
     Or it could modify the number of decimals shown.
     */
    ...
    open func updateLabelText() {
        ...
        lowerLabelTextSize = (lowerLabel.string as? NSString)?
            .size(withAttributes: [NSAttributedString.Key.font: UIFont.systemFont(ofSize: labelFontSize)])
        upperLabelTextSize = (upperLabel.string as? NSString)?
            .size(withAttributes: [NSAttributedString.Key.font: UIFont.systemFont(ofSize: labelFontSize)])
    }
    ...
    /**
     Triggers on touch of the `RangeSlider` and checks whether either of the slider buttons have been touched and sets their `highlighted`
     property to true.
    ...
    override open func beginTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        ...
        if !lowerKnob.isHidden && lowerKnob.frame.contains(previousLocation) && upperKnob.frame.contains(previousLocation) {
        ...
        if !lowerKnob.isHidden && lowerKnob.frame.contains(previousLocation) {
    ...
    override open func continueTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        ...
        var deltaValue = (maximumValue - minimumValue) * deltaLocation / Double(bounds.width - aKnobSize)
        ...
        if stepValue != 0 {
            deltaValue = (deltaValue.sign == .minus ? -1 : 1) * max(1, round(abs(deltaValue) / stepValue)) * stepValue
        }
    ...
    func positionForValue(_ value: Double) -> CGPoint {
        ...
        let knobDeltaX: CGFloat = (aKnobSize / 2) - RangeSliderKnob.KnobDelta
        let knobDeltaWidth: CGFloat = -(aKnobSize - (RangeSliderKnob.KnobDelta * 2))
...
class RangeSliderTrack: CALayer {
    ...
    override func draw(in ctx: CGContext) {
        if let slider = rangeSlider {
            ...
            let lowerValue = slider.lowerKnob.isHidden ? -1 : slider.lowerValue
            let lowerValuePosition = max(0, slider.positionForValue(lowerValue).x)
...
public class RangeSliderKnob: CALayer {
    ...
    override public func draw(in ctx: CGContext) {
        if let slider = rangeSlider {
            ...
            if rangeSlider?.knobHasShadow == true {
