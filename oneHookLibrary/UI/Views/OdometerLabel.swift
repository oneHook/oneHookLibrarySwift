...
public class OdometerLabel: BaseView {
    ...
    public func setNumber(_ number: String, animated: Bool) {
        let result = decodeSegments(from: number)
        ...
        if animated {
            ...
            CATransaction.begin()
            CATransaction.setValue(kCFBooleanTrue, forKey: kCATransactionDisableActions)
            let currResult = decodeSegments(from: self.number)
    ...
    private func recycleUnusedLayersIfAny() {
        let result = decodeSegments(from: self.number)
...
private func decodeSegments(from: String) -> (segments: [Segment], digitCount: Int, textCount: Int) {
